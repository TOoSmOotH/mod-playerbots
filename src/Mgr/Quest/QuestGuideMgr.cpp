...
{
    if (!sBetterQuestingConfig->IsEnabled() || !sBetterQuestingConfig->IsQuestGuideEnabled())
    {
...
{
    if (!player || !sBetterQuestingConfig->IsQuestGuideZoneTransitionEnabled())
        return false;
...
int32 QuestGuideMgr::GetGuideQuestPriorityBonus(Player* player, uint32 questId) const
{
    ...
    const QuestGuideProgress& progress = progressIt->second;
    int32 baseBonus = sBetterQuestingConfig->GetGuidePriorityBonus();
...
void QuestGuideMgr::SyncPlayerProgress(Player* player)
{
    ...
    while (true)
    {
        ...
        if (!player->GetQuestRewardStatus(step->questId))
        {
            ...
            {
                if (sBetterQuestingConfig->IsQuestGuideSkipUnavailableEnabled())
                {
                    ++stepOrder;
                    continue;
                }
            }
            break;  // Found first incomplete step
        }
...
void QuestGuideMgr::SavePlayerProgress(Player* player)
{
    ...
    CharacterDatabase.Execute(
        "REPLACE INTO playerbots_questing_player_guide_progress (guid, guideId, currentStep) VALUES ({}, {}, {})",
        progress.guid, progress.guideId, progress.currentStep);
}

void QuestGuideMgr::LoadPlayerProgress(uint32 guid)
{
    QueryResult result = CharacterDatabase.Query(
        "SELECT guideId, currentStep FROM playerbots_questing_player_guide_progress WHERE guid = {}",
        guid);
    ...
    LOG_DEBUG("module", "BetterQuesting: Loaded progress for guid {}: guide {}, step {}",
        guid, progress.guideId, progress.currentStep);
}
