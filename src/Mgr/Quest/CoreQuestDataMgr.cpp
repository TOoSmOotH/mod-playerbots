...
{
    if (!sBetterQuestingConfig->IsEnabled())
    {
...
std::vector<uint32> CoreQuestDataMgr::GetRecommendedQuests(Player* player, uint32 zoneId, uint32 limit)
{
    ...
    for (auto const& [questId, quest] : questTemplates)
    {
        ...
        if (sBetterQuestingConfig->IsPrerequisiteCheckEnabled() &&
        ...
        if (sBetterQuestingConfig->IsQuestChainAwarenessEnabled())
        {
        ...
        if (sBetterQuestingConfig->IsQuestGuideEnabled() && sQuestGuideMgr->IsDataLoaded())
        {
            int32 guideBonus = sQuestGuideMgr->GetGuideQuestPriorityBonus(player, questId);
            score += guideBonus;

            // In strict mode, filter out non-guide quests
            if (sBetterQuestingConfig->IsQuestGuideStrictMode() && guideBonus == 0)
            {
                if (sQuestGuideMgr->GetPlayerProgress(player->GetGUID().GetCounter()) != nullptr)
                {
