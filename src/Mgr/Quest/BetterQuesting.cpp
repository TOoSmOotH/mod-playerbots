...
namespace BetterQuesting
{
    bool IsEnabled()
    {
        return sBetterQuestingConfig->IsEnabled() && sCoreQuestDataMgr->IsDataLoaded();
    }

    bool GetQuestPOIFallback(uint32 questId, int32 objectiveIndex, uint32 mapId,
                             std::vector<SpawnPoint>& positions)
    {
        if (!IsEnabled() || !sBetterQuestingConfig->IsPOIFallbackEnabled())
            return false;

        bool result = sCoreQuestDataMgr->GetQuestPOI(questId, objectiveIndex, mapId, positions);

        if (result && sBetterQuestingConfig->IsLogFallbackEnabled())
        {
    ...
    std::vector<SpawnPoint> GetNpcSpawns(uint32 npcId, uint32 mapId)
    {
        ...
        return sCoreQuestDataMgr->GetNpcSpawns(npcId, mapId);
    }
    ...
    std::vector<SpawnPoint> GetObjectSpawns(uint32 objectId, uint32 mapId)
    {
        ...
        return sCoreQuestDataMgr->GetObjectSpawns(objectId, mapId);
    }
    ...
    std::vector<SpawnPoint> GetQuestStarterLocations(uint32 questId, uint32 mapId)
    {
        ...
        return sCoreQuestDataMgr->GetQuestStarterLocations(questId, mapId);
    }
    ...
    std::vector<SpawnPoint> GetQuestEnderLocations(uint32 questId, uint32 mapId)
    {
        ...
        return sCoreQuestDataMgr->GetQuestEnderLocations(questId, mapId);
    }

    bool ArePrerequisitesMet(Player* player, uint32 questId)
    {
        if (!IsEnabled() || !sBetterQuestingConfig->IsPrerequisiteCheckEnabled())
            return true; // Default to true if disabled

        return sCoreQuestDataMgr->ArePrerequisitesMet(player, questId);
    }

    uint32 GetNextQuestInChain(uint32 questId)
    {
        if (!IsEnabled() || !sBetterQuestingConfig->IsQuestChainAwarenessEnabled())
            return 0;

        return sCoreQuestDataMgr->GetNextQuestInChain(questId);
    }
    ...
    std::vector<uint32> GetRecommendedQuests(Player* player, uint32 zoneId, uint32 limit)
    {
        ...
        return sCoreQuestDataMgr->GetRecommendedQuests(player, zoneId, limit);
    }
    ...
    ZProbeResult ProbeMultiLevelZ(Unit* unit, float x, float y, float maxProbeDepth)
    {
        ...
        if (!sBetterQuestingConfig->IsMultiLevelZProbeEnabled())
            return result;
        ...
        else if (sBetterQuestingConfig->IsLogFallbackEnabled())
        {
                                                       ...
                                                       float maxDistance)
    {
        ...
        for (const auto& spawn : spawns)
        {
            ...
            ZProbeResult zResult = ProbeMultiLevelZ(unit, spawn.x, spawn.y,
                sBetterQuestingConfig->GetMaxZProbeDepth());
            ...
            if (zResult.isValid)
            {
                ...
                if (zResult.isUnderground && sBetterQuestingConfig->IsLogFallbackEnabled())
                {
            ...
            if (pathable.isPathable || !sBetterQuestingConfig->IsPathValidationEnabled())
            {
        ...
        if (sBetterQuestingConfig->IsLogFallbackEnabled() && !result.empty())
        {
                                  ...
                                  SpawnPoint& outSpawn, float& outPathLength)
    {
        ...
        auto pathableSpawns = GetPathableSpawns(player, positions, sBetterQuestingConfig->GetMaxSpawnDistance());
        ...
        {
            if (ps.isPathable || !sBetterQuestingConfig->IsPathValidationEnabled())
            {
                outSpawn = ps.spawn;
    ...
    {
        if (!unit || !sBetterQuestingConfig->IsApproachWaypointsEnabled())
            return false;
        ...
        auto waypoints = sCoreQuestDataMgr->GetZoneApproachWaypoints(zoneId, mapId);
        ...
        for (const auto& wp : waypoints)
        {
            ...
            if (wp.waypointOrder > bestEntrance->waypointOrder &&
                wp.waypointOrder <= bestEntrance->waypointOrder + sBetterQuestingConfig->GetMaxApproachWaypoints())
            {
                                                         ...
                                                         uint32 zoneId, float maxDistance)
    {
        ...
        for (const auto& ps : pathableSpawns)
        {
            ...
            else if (sBetterQuestingConfig->IsApproachWaypointsEnabled() && zoneId > 0)
            {
    ...
    {
        return sBetterQuestingConfig->IsEnabled() &&
               sBetterQuestingConfig->IsQuestGuideEnabled() &&
               sQuestGuideMgr->IsDataLoaded();
    }
    ...
    uint32 GetCurrentGuideQuest(Player* player)
    {
        ...
        QuestGuideStep const* step = sQuestGuideMgr->GetCurrentStep(player);
    ...
    std::vector<uint32> GetGuidedQuests(Player* player, uint32 limit)
    {
        ...
        auto steps = sQuestGuideMgr->GetAvailableSteps(player, limit);
    ...
    bool ShouldChangeZone(Player* player, uint32& targetZoneId)
    {
        ...
        return sQuestGuideMgr->ShouldTransitionZone(player, targetZoneId);
    }
    ...
    int32 GetGuidePriorityBonus(Player* player, uint32 questId)
    {
        ...
        return sQuestGuideMgr->GetGuideQuestPriorityBonus(player, questId);
    }
    ...
    bool IsQuestInActiveGuide(Player* player, uint32 questId)
    {
        ...
        return sQuestGuideMgr->IsQuestInActiveGuide(player, questId);
    }
    ...
    void InitializePlayerGuide(Player* player)
    {
        ...
        if (sBetterQuestingConfig->IsAutoGuideSelectionEnabled())
        {
            QuestGuideData const* bestGuide = sQuestGuideMgr->GetBestGuideForPlayer(player);
            if (bestGuide)
            {
                sQuestGuideMgr->SetPlayerGuide(player, bestGuide->guideId);
                sQuestGuideMgr->SyncPlayerProgress(player);
